...
\end{center}
	...
	\begin{aligned}
		...
		& = \Prob\left(Bin\left(n, \frac{1}{2}\right)\le\frac{n}{2}-|s|\right) + \Prob\left(Bin\left(n{2}, \frac{1}{2}\right)\ge\frac{n}{2}+|s|\right).
	\end{aligned}
	\]
\end{example}
...
\begin{remark}
	...
	\[
	\Omega = \frac{L(0)}{\max\;\{L(0),L(1)\}}.
	\]
	Für $ c<1 $ ist $ \Omega\le c \Leftrightarrow \frac{L(0)}{L(1)} \le c $.
\end{remark}
...
\begin{proof}
	...
	\[
	\alpha = \Prob(\Omega\le c\;|H_0) = \Prob\left(\frac{f_0(x)}{f_1(x)}\le c \;|H_0\right).
	\]
	...
	\begin{aligned}
		...
		& \le \Prob(d_{LR}=1\;|H_1) - \frac{1}{c}\int_{x\colon d(x)=0, d_{LR}(x)=1} f_0(x) dx + \int_{x\colon d(x)=1, d_{LR}(x)=0} f_0(x) dx \\
...
\begin{corollary}
	Seien $ X_1,\ldots,X_n $ i.i.d. $ N(\mu,\sigma^2), \quad \mu \in \left(-\infty,\mu\right], \quad \sigma^2 > 0  $ bekannt.
...
\begin{proof}
	...
	\begin{itemize}
		...
		\[
		\begin{aligned}
			& & \tilde{\Omega} & = \frac{L(\mu_0)}{\max\;\{L(\mu_0),L(\mu_1)\}} & \le \tilde{c} & < 1 \\
...
\begin{remark}
	Bemerkung: Die obrige Aussage gilt auch für rechtsseitige Alternativen $ (\mu>\mu_0) $, sowie im Fall unbekannter Varianz, d.h. für den t-Test.
\end{remark}
